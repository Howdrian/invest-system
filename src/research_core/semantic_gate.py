...
_QUALITATIVE_LEVEL_RE = re.compile(
    r"(?:极其|相对|整体)?(?:平稳|温和|健康|宽松|紧张|极低|极高)(?:区间|水平|环境|状态)?"
...
_SYMBOL_RE = re.compile(r"(?<![A-Z0-9])(?:AAPL|HK\d{4,5}|\d{6})(?![A-Z0-9])", re.I)
...
_SYSTEMIC_MARKET_STRESS_RE = re.compile(
    r"系统性(?:去杠杆|风险收缩|流动性(?:危机|收缩)|抛售|踩踏|深幅调整)"
...
_MARKET_COUNT_PATTERNS = (
    ("up_count", re.compile(r"(?:上涨|收涨)(?:家数)?[^0-9]{0,8}(\d{2,5})\s*家")),
    ("down_count", re.compile(r"(?:下跌|收跌)(?:家数)?[^0-9]{0,8}(\d{2,5})\s*家")),
...
def validate_claim(claim: AtomicClaim, evidence: Iterable[EvidenceFact]) -> ClaimValidation:
    ...
    inferred_subjects = {item.upper() for item in _SYMBOL_RE.findall(claim.text)}
    supported_subjects = {str(row.subject or "").upper() for row in accepted if row.subject}
    ...
    if _VOLUME_EXPANSION_RE.search(text) and volume_ratios and max(volume_ratios) < 1.0:
        reasons.append("volume_expansion_contradicted_by_evidence")
    if _VOLUME_CONTRACTION_RE.search(text) and volume_ratios and min(volume_ratios) > 1.0:
    ...
    if _ROE_VALUATION_RE.search(text):
        valuation_benchmark_support = any(
            any(
                token in f"{row.id} {row.metric} {row.value}".upper()
                for token in ("VALUATION_HISTORY", "VALUATION_PERCENTILE", "PEER_VALUATION", "PEER_COMPARISON")
            )
            for row in accepted
        )
    ...
    if _GDP_LEVEL_GROWTH_RE.search(text):
...
def _soften_gdp_level_growth_language(text: str) -> str:
    return re.sub(
        r"(?:结合)?[^。；]{0,48}GDP[^。；]{0,48}(?:万亿美元|万亿|总量|基数)[^。；]{0,48}(?:温和增长|稳健增长|增长态势)[^。；]*",
        "GDP总量本身不能证明增长速度；现有指标需结合增长率、就业与信用数据判断",
        str(text or ""),
        flags=re.I,
    )
...
def _volume_ratios_for_claim(claim: AtomicClaim, accepted: Sequence[EvidenceFact]) -> List[float]:
    ...
    if claim_subject and claim_subject not in aggregate_subjects:
        target_subjects.add(claim_subject)
    target_subjects.update(item.upper() for item in _SYMBOL_RE.findall(claim.text))
    ...
    if target_subjects:
        relevant = [row for row in rows_with_ratio if str(row.subject or "").strip().upper() in target_subjects]
    else:
...
def _subject_matches(claim_subject: str, row: EvidenceFact) -> bool:
    ...
    expected = str(claim_subject or "").strip().upper()
    actual = str(row.subject or "").strip().upper()
