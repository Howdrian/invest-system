...
def _load_local_env(path: Path = Path(".env")) -> None:
    """Load simple KEY=VALUE entries for standalone local scripts.

    Keep process env priority: exported variables are never overwritten.
    This avoids marking configured local keys (for example FRED_API_KEY) as
    missing when market_cycle is run directly.
    """
    if not path.exists():
        return
...
def _render_one_screen_body(payload: Dict[str, Any]) -> str:
    ...
    return f"""
<section class="hero">
  <div><span class="label">统一看盘 · 一屏总览</span><h1>{html.escape(_reader_label(str(strategy.get('regime'))))}</h1><p>{html.escape(str((strategy.get('strategy') or {}).get('headline') or ''))}</p></div>
  <div class="kpi"><small>Macro</small><b>{html.escape(str(payload.get('macro_status')))}</b><span>{html.escape(str(reason))}</span></div>
  <div class="kpi"><small>数据</small><b>{html.escape(_reader_label(str(health.get('usability_verdict'))))}</b><span>trade: {html.escape(_reader_label(str(health.get('trade_review_usability'))))}</span></div>
</section>
{indicators_table}
{gov_html}
<section class="card"><h2>今日边界</h2><ul><li>只读看盘，不自动交易。</li><li>个股交易前必须经过个股深评、红蓝、评分、CIO 和人工确认。</li><li>可选源失败只降权；关键源不可用才阻断交易审查。</li></ul></section>
<section class="card"><h2>宏观/候选摘要</h2><ul><li>宏观报告：{html.escape(str(macro_review.get('status') or '-'))} · {html.escape(str(macro_review.get('confidence') or '-'))}</li><li>深评候选：{len(queue.get('candidates') or [])}，自动深评：{len(queue.get('auto_governed_candidates') or [])}</li><li>Prediction market：{html.escape(str(payload.get('prediction_market_status') or '-'))}</li></ul></section>
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    allow_macro_network = bool(args.refresh_macro or os.getenv("FRED_API_KEY", "").strip())
    macro_context = MacroContextService().get_context(
        allow_network=allow_macro_network,
