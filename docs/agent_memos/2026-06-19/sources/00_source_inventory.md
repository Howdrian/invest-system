...
source_status_v1；用于说明每个数据源状态、失败原因、影响范围和下一步。
...
| 数据源 | 今天状态 | 失败原因 | 影响哪个分析结论 | 是否阻断交易审查 | fallback 是否启用 | 下一步怎么修 |
|---|---|---|---|---|---|---|
| src.intel.market_heat | REFRESHED | 无 | 影响 A股热榜、公告、候选池和深评队列 | 不阻断，只降权 | 未启用 | 保持监控，下一轮自动复查 |
...
| 数据源 | 今天状态 | 失败原因 | 影响哪个分析结论 | 是否阻断交易审查 | fallback 是否启用 | 下一步怎么修 |
|---|---|---|---|---|---|---|
| src.intel.candidate_selector | REFRESHED | 无 | 影响新闻、研报、公告原文和催化剂判断 | 不阻断，只降权 | 未启用 | 保持监控，下一轮自动复查 |
| src.intel.candidate_selector | REFRESHED | 无 | 影响新闻、研报、公告原文和催化剂判断 | 不阻断，只降权 | 未启用 | 保持监控，下一轮自动复查 |
| reports/report_YYYYMMDD.md | REFRESHED | 无 | 影响新闻、研报、公告原文和催化剂判断 | 不阻断，只降权 | 未启用 | 保持监控，下一轮自动复查 |

## 持仓

| 数据源 | 今天状态 | 失败原因 | 影响哪个分析结论 | 是否阻断交易审查 | fallback 是否启用 | 下一步怎么修 |
|---|---|---|---|---|---|---|
| src.intel.portfolio_holdings | REFRESHED | 无 | 影响持仓轻量复核、浮盈亏和是否进入 governed | 不阻断，只降权 | 未启用 | 保持监控，下一轮自动复查 |
